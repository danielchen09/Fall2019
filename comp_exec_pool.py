...
class Model:
    def __init__(self, w1=None, w2=None, load=False):
        self.inputSize = 8
        self.hiddenSize = 4
        self.outputSize = 1
        self.score = 0
        ...
        if w1 is None or w2 is None:
            if load:
                self.w1 = loadWeight("w1")
                self.w2 = loadWeight("w2")
            else:
    ...
    def getAction(self, rgb, paddleA, paddleB, ball, reward, done):
        if self.lastPos is None:
            ...
            input = np.concatenate((pos - np.array(self.lastPos), pos)).reshape(-1, 1)
            self.lastPos = pos
    ...
    def runComp(self):
        for epoch in range(self.epochs * self.modelSize):
            ...
            if self.current == self.modelSize:
                ...
                saveWeight(m1_w1_new, "w1")
                saveWeight(m1_w2_new, "w2")
