...
class Model:
    ...
    def getAction(self, rgb, paddleA, paddleB, ball, reward, done):
        if self.lastPos is None:
            self.lastPos = (paddleA.y, paddleB.y, ball.y, ball.x)
            return 0
        input = (np.array([paddleA.y, paddleB.y, ball.y, ball.x]) - np.array(self.lastPos)).reshape(-1, 1)
...
class Game():
    ...
    def crossOverWeight(self, w1, w2):
        mask0 = np.random.randint(0, 2, size=(w1.shape[0]))
        mask1 = np.random.randint(0, 2, size=(w1.shape[1]))
        temp = np.array(w1[mask0, mask1], copy=True)
        w1[mask0, mask1] = w2[mask0, mask1]
        w2[mask0, mask1] = temp
    ...
    def runComp(self):
        for epoch in range(self.epochs * self.modelSize):
            ...
            if self.current == self.modelSize:
                ...
                self.saveWeight(m1_w1_new, "w12")
                self.saveWeight(m1_w2_new, "w22")
